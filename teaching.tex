...
This game has a dynamic component, as the government and the secessionists move after seeing the patron's choice. Further, we assume that the players have symmetric information about payoffs; that is, all the players know the payoffs associated with each outcome. We will also assume that the players know each other well enough to be able to correctly predict each other's behavior; this is likely to be the case when the same leaders have been in place for a while so that they have interacted repeatedly. As this is a high-stakes `game,' all the players have strong incentives to seek out information and think carefully about what the other players will do before choosing the strategy that will maximize their expected payoffs, and this is common knowledge amongst the players. Finally, no commitment devices are available to enforce players' promises, so the players cannot make commitments that are incredible. \\

These assumptions (the game is dynamic, information in symmetric, and the actors have common knowledge of sequential rationality and can correctly predict each other's behavior) imply that Subgame Perfect Nash Equilibrium (SPNE) is the appropriate solution concept for this strategic situation (Watson p. 45 and p. 189).
