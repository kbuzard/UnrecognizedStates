...
Ben
\begin{itemize}
	\item R1: Finish \& Double Check Fearon reference
		\item add appropriate language to memo if necessary

	\item R1: Talk to Kristy about whether we want to offer a numerical example a la what R1 requests.
	
	\item R1: Tone down our claims about being novel and unique
	
	\item R1: Double check that we have fixed the typo identified by R1
	
	\item R3: Read the literature referenced by R3 and add some cites
	
	\item R3: Read Berg 2009 in particular and engage it at least briefly in the paper.
