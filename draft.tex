...
Future payoffs are discounted with parameters $\delta_i$, where 1/$\delta_i$ is player $i$'s discount rate for $i \in (g, s, c, p)$, $0\leq\delta_i\leq1$. Therefore payoffs for the entire game for player $i\in \{s, g, p, c\} $ can be expressed by the discounted stream of payments $\Sigma_{n=1}^{\infty} U_{in} \delta_i^{n-1}$. 
