...
We will again set $R_p^{SQ}$ at its largest value of $\frac{\mu + \beta}{1-\de}$ as this is most restrictive. In this only case of interest, we have $\frac{\alpha (1-p_{1s}) + \lambda p_{1s} + \mu + \beta}{1 - \de}> 0$. Note that the first term is negative while the rest are positive.

Multiplying Condition 3 by $-(1-p_{1s})$, we have $-\beta \cdot (1-p_{1s}) \geq \alpha \cdot (1-p_{1s})$. Using this with Condition 4, we have $- \nu p_{1s} - \beta (1-p_{1s}) \geq \alpha (1-p_{1s}) +\lambda p_{1s} + \mu + \beta> 0 \ \forall p_{1s},p_{2s}$ and therefore also
\[
  \textstyle -\left(\frac{\nu p_{1s} + \beta (1-p_{1s})}{1 -\de}\right) \geq \frac{\alpha (1-p_{1s})+ \lambda p_{1s} + \mu + \beta}{1 - \de}> 0 \ \forall p_{1s},p_{2s}
\]
