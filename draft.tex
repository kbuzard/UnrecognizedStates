...
\begin{enumerate}
\item \textit{For both players $g$ and $s$, $Q_{it} \geq L_{it} \ \forall t$: in each period, remaining in the status quo is better than ceding.}

\item \emph{For both players $g$ and $s$, $\frac {Q_{it}}{1-\delta} \geq  -\zeta_i+\frac{W_{it}(p_1) + L_{it}(1-p_1)}{1-\delta} \ \forall n$: in each period, the expected outcome under war is worse than the status quo.}
