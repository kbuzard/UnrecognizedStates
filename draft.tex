...
\subsection{Interaction within a Period}
\label{sec:structure}
...
The payoff functions and all parameters, including probabilities in the war lottery, are common knowledge for all players, and actions are immediately observed by all players at the time they occur. There are an infinite number of discrete periods $t=1,2,\ldots$, with play proceeding until an absorbing state is reached.

Within each period, there is first an investment stage in which first the patron and then the international community each make decisions about how much to invest in altering the payoffs of the `inside actors'---the secessionists and the home government. Then, the inside actors play a ``conflict stage game'' in which they simultaneously choose whether to cede the issue of status, fight, or maintain the status quo.

The payoffs at the end of a period are determined by these actions and six state variables, which keep track of the evolving value of the status quo ($Q_s^t$ and $Q_g^t$), losing the issue of recognition ($L_s^t$ and $L_g^t$) and winning the issue of recognition ($W_s^t$ and $W_g^t$) to the secessionists and government respectively. The game continues into the next period if the inside players selected (status quo, status quo) or (cede, cede). Otherwise, the game ends in an absorbing state as described below.
...
The other payoffs for the inside actors are also modeled as state variables. However, they remain unchanged from period to period unless players $p$ or $c$ make an investment, to be precise $L_s^t = L_s^{t-1} - R_p^t + R_c^t$ where $L_s^t$ is the per-period payoff to the secessionists from ceding the issue of status. The full set of payoffs for the inside actors are described in Section~\ref{sec:stage}.

Both because of the presence of absorbing states and state variables, this is a dynamic game rather than a standard repeated game.
...
In each period $t$, play proceeds as follows:
