...
The payoffs at the end of a period are determined by these actions and six state variables, which keep track of the per-period value of the status quo ($Q_s^t$ and $Q_g^t$), losing the issue of recognition ($L_s^t$ and $L_g^t$) and winning the issue of recognition ($W_s^t$ and $W_g^t$) to the secessionists and government respectively. The game continues into the next period if the inside players selected (status quo, status quo) or (cede, cede). Otherwise, the game ends in an absorbing state as described below.
